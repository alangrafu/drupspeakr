...
class Utils{
  ...
  public static function processDocument($viewFile, $base, $data){
  	...
  	return Utils::showView($base, $data, $viewFile);  	
  }
  ...
  public static function queryFile($modelFile, $e, &$rPointer){
  	...
  	$uri = $base['this']['value'];
  	$data = array();
  	$dir = $conf['home'].$base['model']['directory'];
             
  	
 	if(!is_dir($dir.$modelFile)){
 	  ...
 	  $fnc = Haanga::compile(file_get_contents($dir.$modelFile));
  	  $query = $fnc($vars, TRUE);
 	  if(is_object($base)){
  ...
  public static function showView($baseData, $data, $view){
	...
	//if(is_file($base['view']['directory'].$view)){
	  $res .= Haanga::Load($view, $vars, TRUE);
	  return $res;
	  //}else{
